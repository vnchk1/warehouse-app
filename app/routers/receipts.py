...
def create_receipt(receipt_in: ReceiptCreate, db: Session = Depends(get_db)):
    # Проверка бизнес-правил (ссылочная целостность)
    if not db.get(Material, receipt_in.material_id):  # Обновленный синтаксис
        raise HTTPException(status_code=400, detail="Material not found")
    if not db.get(Supplier, receipt_in.supplier_id):  # Обновленный синтаксис
...
def get_receipt(receipt_id: int, db: Session = Depends(get_db)):
    receipt = db.get(Receipt, receipt_id)  # Обновленный синтаксис
    if not receipt: 
